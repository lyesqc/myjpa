...
public class EnvoitransAction extends DispatchAction {
...
/* public ActionForward afficher(ActionMapping mapping, ActionForm form,
  ...
  Transfert1Form transfert1Form = (Transfert1Form) form;// TODO Auto-genera
  ...
  rs=st.executeQuery("select distinct gss_warehouse.whs_adresse from gss_actor,gss_droit_acces,gss_profil_actor,gss_warehouse where gss_profil_actor.profil_act_libelle in ('MAGASINIER REGIONAL','MAGASINIER CENTRAL') and gss_profil_actor.profil_act_id=gss_droit_acces.profil_act_id and gss_warehouse.whs_id=gss_droit_acces.whs_id and gss_warehouse.whs_status='A' and gss_droit_acces.act_id=gss_actor.act_id and gss_actor.act_name='"+user+"'");
  
 
  while (rs.next())
     {
                  
     String mag = rs.getString(1).trim();
     ...
     listmag.add(mag);
      ...
      transfert1Form.setListmagasin(listmag);
     /* 
     list=transfert1Form.getListeordre();
      list.clear();
      transfert1Form.setListeordre(list); 
*/

  
// return mapping.findForward("listordretrans");
//}
 ...
 public ActionForward recherche(ActionMapping mapping, ActionForm form,
  ...
  for ( int i=0;i<priv.size();i++){
   ...
   if (profi.getProf().equals("AJOUTER ENVOI TRANSFERT")){
    whs=profi.getWhs();
   System.out.println("whs4 transfert :"+whs);
   //request.getSession().setAttribute("werhs",whs4);
   break;}}
  
 
 request.getSession().setAttribute("warehouse",whs);*/
  
  
   
  
  
  whh=transfert1Form.getMagasin();
  ...
  System.out.println("je suis ds recherche de envoitrans");
  request.getSession().setAttribute("warehousenvoi",whs); 
  
  
 st=con.createStatement();
//  st=con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
  System.out.println("select distinct tr.t_r_trans_type||'-'||tr.t_r_trans_whs||'-'||tr.t_r_trans_no,tr.t_r_trans_type,tr.t_r_trans_whs,tr.t_r_trans_no,d.whs_adresse from gss_transaction_range tr, gss_range r, gss_transaction t, gss_warehouse d where tr.t_r_rng_id=r.rng_id and tr.t_r_rng_act_id=r.act_id and tr.t_r_trans_no=t.trans_no and tr.t_r_trans_type=t.trans_type and tr.t_r_trans_whs=t.whs_id and  tr.t_r_trans_type='OTR' and r.rng_inactif='A' and d.whs_id(+)=t.whs_dest and  r.rng_whs='"+whs+"'and r.rng_transfert='O'");
  //rs=st.executeQuery("select distinct tr.t_r_trans_type||'-'||tr.t_r_trans_whs||'-'||tr.t_r_trans_no,tr.t_r_trans_type,tr.t_r_trans_whs,tr.t_r_trans_no,d.whs_adresse,tr.t_r_date  from gss_transaction_range tr, gss_range r, gss_transaction t, gss_warehouse d where tr.t_r_rng_id=r.rng_id and tr.t_r_rng_act_id=r.act_id and tr.t_r_trans_no=t.trans_no and tr.t_r_trans_type=t.trans_type and tr.t_r_trans_whs=t.whs_id and  tr.t_r_trans_type='OTR' and r.rng_inactif='A' and d.whs_id(+)=t.whs_dest and  r.rng_whs='"+whs+"'and r.rng_transfert='O' order by  tr.t_r_date ");
  rs=st.executeQuery("select distinct  t.trans_type||'-'||t.whs_id||'-'||t.trans_no,t.trans_type,t.whs_id,t.trans_no,d.whs_adresse from (select distinct tr.t_r_trans_type as typ_trans,tr.t_r_trans_whs as whs ,MAX(tr.t_r_trans_no) as code,tr.t_r_rng_id,tr.t_r_rng_act_id from gss_transaction_range tr, gss_range r where tr.t_r_rng_id=r.rng_id and tr.t_r_rng_act_id=r.act_id  and  tr.t_r_trans_type='OTR' and r.rng_inactif='A'  and  r.rng_whs='"+whs+"'and r.rng_transfert='O' GROUP BY tr.t_r_trans_type,tr.t_r_trans_whs,tr.t_r_rng_id,tr.t_r_rng_act_id)aa,gss_warehouse d,gss_transaction t where t.trans_no=aa.code and t.trans_type=aa.typ_trans and t.whs_id=aa.whs and t.whs_dest=d.whs_id(+)  ");
    while (rs.next()){
 Ordre ordre = new Ordre();
 String nn=rs.getString(1).trim();
 ordre.setOrdre(nn);
...
transfert1Form.setListeordre(list);
 ...
 public ActionForward ouvrir(ActionMapping mapping, ActionForm form,
   HttpServletRequest request, HttpServletResponse response) throws SQLException,IOException, ServletException  {
  Transfert1Form transfert1Form = (Transfert1Form) form;
...
/*try{
rs1=st1.executeQuery("select max(trans_no) from gss_transaction  where (trans_type='TRA') and (whs_id='"+whs+"')");
 ...
 */
/*String code_trans = "TRA"+"-"+whs+"-"+maxi;
...
while (rs.next())
 {
  ...
  list.add(rang);
     ...
     transfert1Form.setResulrang(list);
     transfert1Form.setListajouter(list2);
     transfert1Form.setNomArticle(listart);
     transfert1Form.setListprof(listprof);
  //   transfert1Form.setTrans(code_trans);
     transfert1Form.setDoc("");
 
     transfert1Form.setNote("");
     transfert1Form.setProfil("");
     ...
     transfert1Form.setHiddenTest("");  
    
	    	con.close();
    
 return mapping.findForward("envoitrans");
}
 ...
 // ArrayList mm1 = new ArrayList();
  ...
  transfert1Form.setDebut(nn2);
  transfert1Form.setFin(nn3);
  transfert1Form.setArticle(nn);
  transfert1Form.setProfil(nn1);
  transfert1Form.setAct_id(act_id);
  transfert1Form.setRng_id(rng_id);
 ...
 public ActionForward ajouter1(ActionMapping mapping, ActionForm form,
  ...
  list=transfert1Form.getListajouter();
  listGrand = transfert1Form.getResulrang();
  
  String act_id2=null,rng_id2=null;  
  
  String nn=transfert1Form.getArticle();
  String nn1=transfert1Form.getProfil();
  String nn2=transfert1Form.getDebut();
  String nn3=transfert1Form.getFin();
  String rng_id=transfert1Form.getRng_id();
  String act_id=transfert1Form.getAct_id();
System.out.println(rng_id);
  System.out.println(act_id);
...
long  qte=R2-R1+1;
 ...
 if (qte<=0){
     transfert1Form.setHiddenTest("fin inferieur debut"); 
      System.out.println("le fin est< debut");
      
     transfert1Form.setArticle("");
  transfert1Form.setProfil("");
  transfert1Form.setDebut("");
  transfert1Form.setFin("");
 ...
 if (size==0){
      ...
      transfert1Form.setHiddenTest("range n'est pas dans la coll grande"); 
  ...
  /* rang2.setAct_id(act_id);
   ...
   rang2.setRng_id(rng_id2);
    ...
    trouver=true;
    transfert1Form.setListajouter(nouv);
    transfert1Form.setHiddenTest("");
      //break;
     ...
     /* rang2.setAct_id(act_id);
      ...
      rang2.setRng_id(rng_id2);
       ...
       ///// si la collection est non vide
          ...
          {
           transfert1Form.setHiddenTest("range dans la coll"); 
 ...
 /*   rang2.setAct_id(act_id);
    rang2.setRng_id(rng_id);*/  
      ...
      transfert1Form.setListajouter(list); 
    }
...
public ActionForward Retirer(ActionMapping mapping, ActionForm form,
 ...
 public ActionForward valider(ActionMapping mapping, ActionForm form,
  ...
  List listt;
  String whh=null,max=null,whstrans=null;
  ArrayList priv =  new ArrayList() ;
  ...
  try{
  	  rs6=st6.executeQuery("select max(trans_no) from gss_transaction  where (trans_type='TRA') and (whs_id='"+whh+"')");
  ...
  String note=transfert1Form .getNote();
  System.out.println(note);
  
  String doc=transfert1Form .getDoc();
  ...
  st3=null;rs3=null;
  st3=con.createStatement();
  rs3=st3.executeQuery("select  gss_transaction.whs_dest from gss_transaction where gss_transaction.trans_no='"+code+"'  and gss_transaction.trans_type='OTR' and gss_transaction.whs_id='W00'");
  while (rs3.next())
     {
                  
           whstrans = rs3.getString(1).trim();
     System.out.println(whstrans);
      }
  ...
  listt=transfert1Form .getListajouter();
  ...
  /*System.out.println("insert into gss_transaction values ('"+max+"','TRA','"+whh+"',SYSDATE,'"+idact+"','','"+doc+"','"+note+"','','',(select  gss_transaction.whs_dest from gss_transaction where gss_transaction.trans_no='"+code+"'  and gss_transaction.trans_type='OTR' and gss_transaction.whs_id='W00'))");
   h = st.executeUpdate("insert into gss_transaction values ('"+max+"','TRA','"+whh+"',SYSDATE,'"+idact+"','','"+doc+"','"+note+"','','',(select  gss_transaction.whs_dest from gss_transaction where gss_transaction.trans_no='"+code+"'  and gss_transaction.trans_type='OTR' and gss_transaction.whs_id='W00'))");
  */
  ...
  System.out.println(size);
     ...
     transfert1Form.setHiddenTest("collection null");
      ...
      {
   try{
      System.out.println("insert into gss_transaction values ('"+max+"','TRA','"+whh+"',SYSDATE,'"+idact+"','','"+doc+"','"+note+"','','',(select  gss_transaction.whs_dest from gss_transaction where gss_transaction.trans_no='"+code+"'  and gss_transaction.trans_type='OTR' and gss_transaction.whs_id='W00'),'','N')");
      h = st.executeUpdate("insert into gss_transaction values ('"+max+"','TRA','"+whh+"',SYSDATE,'"+idact+"','','"+doc+"','"+note+"','','',(select  gss_transaction.whs_dest from gss_transaction where gss_transaction.trans_no='"+code+"'  and gss_transaction.trans_type='OTR' and gss_transaction.whs_id='W00'),'','N')");
  ...
  // rs2=st2.executeQuery("select rng_start,rng_end,rng_id,act_id,profil_id,art_id,rng_pq,rng_statut,rng_swap,trans_type,whs_id,trans_no,rng_promotion,rng_priorite,rng_packaged,rng_transfert,rng_sortie,rng_reintegration,rng_whs,rng_dist,rng_packaging_whs,rng_sortie_whs,rng_reintegration_whs,rng_transfert_whs from gss_range r where (rng_inactif ='A') and r.rng_transfert=('O') and  r.rng_tpd is null");
    
   rs2=st2.executeQuery("select rng_start,rng_end,rng_id,act_id,profil_id,art_id,rng_pq,rng_statut,rng_tpd,trans_type,whs_id,trans_no,rng_promotion,rng_priorite,rng_packaged,rng_transfert,rng_sortie,rng_reintegration,rng_whs,rng_dist,rng_packaging_whs,rng_sortie_whs,rng_reintegration_whs,rng_transfert_whs from gss_range r where r.rng_id='"+rng_id+"' and r.act_id='"+act_id+"'");
 ...
 String whstransfert=getResult(rs2.getString(24));
    ...
    { 
     ...
     h2 = st7.executeUpdate("insert into gss_range values ('"+res1+"','"+idact+"',decode('"+pfp+"','null',null,'"+pfp+"'),'"+arp+"','"+sta+"','"+ind+"','"+rp+"','"+acp+"','A',SYSDATE,'',decode('"+pq+"','null',null,'"+pq+"'),'"+statu+"','TRA','"+whh+"','"+max+"','','',decode('"+promo+"','null',null,'"+promo+"'),decode('"+prior+"','null',null,'"+prior+"'),'"+p+"','C','"+s+"','"+r+"','"+whp+"',decode('"+dist+"','null',null,'"+dist+"'),decode('"+whspack+"','null',null,'"+whspack+"'),decode('"+whssor+"','null',null,'"+whssor+"'),decode('"+whsrentegr+"','null',null,'"+whsrentegr+"'),'"+whstrans+"' )");
     
     h3 = st9.executeUpdate("insert into gss_transaction_range values ('TRA','"+whh+"','"+max+"','"+res1+"','"+idact+"',SYSDATE)");
 ...
 h8 = st12.executeUpdate("UPDATE gss_range SET rng_inactif = 'I',rng_inactif_date=sysdate WHERE (rng_id='"+rp+"')and (act_id='"+acp+"') and rng_inactif='A' ");
     ...
     if (h8==0)
     {
    	 ...
    	 transfert1Form.setHiddenTest("rang deja touche");
    	 con.rollback();
         return mapping.findForward("envoitrans");
 ...
 h8 = st12.executeUpdate("UPDATE gss_range SET rng_inactif = 'I',rng_inactif_date=sysdate WHERE (rng_id='"+rp+"')and (act_id='"+acp+"') and rng_inactif='A' ");
   ...
   String num="TRA"+"-"+whh+"-"+max;
   request.setAttribute("num", num);
   System.out.println("le num de transaction"+num);
  return mapping.findForward("menu"); 
   
/* editrapport( request,response,"TRA",whh,max);
  
 ActionForward  forward = new RedirectingActionForward();
       forward.setPath("/envoitrans.do?hidden=recherche");
       return forward; */
 ...
 public ActionForward editExcel(ActionMapping mapping, ActionForm form,
		   HttpServletRequest request, HttpServletResponse response) throws SQLException,IOException, ServletException  {
	 Transfert1Form transfert1Form = (Transfert1Form) form;// TODO Auto-generated method stub
		
		List list =transfert1Form.getResulrang();
		String trans="transfert";
